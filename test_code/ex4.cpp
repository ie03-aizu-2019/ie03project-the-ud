...
vector<int> restore(int s,int t,vector<vector<int> > g,vector<Pd> pt,vector<double> dt){
  vector<int> rt;
  while(s!=t){
    rt.pb(t);
    for(int i=0;i<g[t].size();i++){
	 int to=g[t][i];
	 if(dt[to]-(dt[t]-dist(pt[t],pt[to]))<EPS){
	   t=to;
	   break;
	 }
    }
...
vector<double> dijkstra(int s,vector<vector<int> > g,vector<Pd> pt){
...
vector<vector<int> > make_g(int n,int m,vector<Pd> &pt,vector<Pi> &rd,vector<PP> &c){
  vector<vector<int> > g(251111);
...
int main(){
  ...
  vector<vector<int> > g=make_g(n,m,pt,rd,c);
  ...
  for(int i=0;i<q;i++){
    ...
    vector<double> dt=dijkstra(s,g,pt);
    if(dt[t]==INF){
	 cout<<"NA"<<endl;
	 continue;
    }
    ...
    vector<int> rt=restore(s,t,g,pt,dt);
    reverse(all(rt));
